...
def get_raw_images(raw_images_dir=settings.USER_INPUT_PATH):
    ...
    if not raw_images:
        try:
            ...
            print(f'Please add images to “{dir_name}” within the dupported formats and try Again...\n')
            sys.exit()
...
def main():
    try:
        ...
        print('ServerError: Temporary issue with the Tinify API. Please try again later...\n')
    except tinify.ConnectionError:
        print('ConnectionError: A network connection error occurred. Please try again later...\n')
